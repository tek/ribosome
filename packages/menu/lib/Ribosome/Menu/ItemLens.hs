...
import Ribosome.Menu.Combinators (filterEntries, foldEntries, lookupEntryIndex, sortedEntries)
...
getFocusEntry ::
  ...
  Maybe (Entry (Item s))
getFocusEntry s =
  lookupEntryIndex (fromIntegral (s ^. #cursor)) (s ^. entries)

focusEntry ::
  MenuState s =>
  SimpleGetter (WithCursor s) (Maybe (Entry (Item s)))
focusEntry =
  to getFocusEntry
