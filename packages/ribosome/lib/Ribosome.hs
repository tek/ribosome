...
-- listed at @:help api@.
-- These declaration are exposed as an abstract monad, 'MonadRpc', which allows them to be used either in their data
-- form ('RpcCall') or as 'Sem's.
--
-- The data form has the advantage of batching multiple requests into one call to @nvim_call_atomic@ to improve
-- performance.
-- This only works for non-monadic sequences of RPC calls, which can be constructed via 'Applicative' and 'Semigroup'.
--
-- An exposed API function should wrap its contents with a call to 'atomic' to enforce the intermediate conversion to
-- 'RpcCall' for atomic optimization.
-- If the abstract monad @m@ is instantiated as 'Sem', it causes only the individual requests to be constructed as
-- 'RpcCall's.
-- The call to 'atomic' converts the entire program to 'RpcCall' before converting it to the final monad, 'Sem'.
--
-- Take this example:
--
-- > fileBuffers ::
...
-- request.
--
-- The second line uses 'foldMap' to concat multiple requests with the 'Semigroup' for 'RpcCall', resulting in a single
-- request that queries the names of all buffers.
--
...
-- $embed
-- While [remote plugins]("Ribosome#g:execution") are executed from within Neovim, Ribosome can also run Neovim from a
